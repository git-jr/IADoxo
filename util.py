...
class Apps(object):
    ...
    def listarapps(self): # Lista todos aplicativos do dispositivo no formato App/Atividade Principal/ Nome do pacote
        ...
        for i in apps[1]: # Ciclo que organiza as coisas
            cl=Classificador() 
            nomeapp=cl.normalizar(i) # O nome do app deve ficar em minúsculo para agilizar comparações futuramente       
            atividade=apps[1][i]   
        ...
        for j in lapps: # Ciclo para adicionar os pacotes que foram listados no instaciamento da classe
            for i in lpacotes:
                if(i in j[1]):                           
                    self.listaapps[self.listaapps.index(j)].append(i) # O app correspondente recebe o nome do pacote agora                   
                    lpacotes.pop(lpacotes.index(i)) # Retira o pacote para não acabar sendo associado a outro app                           
    ...
    def abrirapp (self,appsele):
        ...
        for i in apps: # Ciclo para achar a atividade principal do aplicativo selecionado
            if(appsele in i[0]) or (i[0] in appsele): # Mesmo se a pessoa não falar o nome do app completo deve funcionar               
...
class Cronos(object):   
    ...
    def tempo(self):
        ...
        meses=["Só pra usar o índice 0","Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"] 
...
class Musicas(object):
    ...
    def locint(self): # Primeiro na memória interna
        os.chdir(os.path.dirname(os.path.abspath(__file__))) # Aponta para o arquivo atual
        os.chdir("..")# Sobe para o diretório raiz do dispositivo # Pasta onde ficam localizados os nomes dos pacotes  
        os.chdir("..") 
    ...
    def locmusica(self, musica): # Busca uma música em todos os arquivos do dispositivo e se ela existir, retorna seu caminho 
    ...
    def tocarmusica(self, caminhomusica):
        ...
        while(brilho!=bmax): # Para a música se o brilho for aumentado para o máximo
            ...
            if(brilho<=bmin): # Um sistema de Interruptor para identificar quando pausar ou despausar a música
                ...
                while(droid.getScreenBrightness()[1]<=bmin): # Prende o usuário nesses lopping até tirar o brilho do mínimo
                    pass
